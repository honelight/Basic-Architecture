...
uns64   dram_access_extra_credit(DRAM *dram,Addr lineaddr, Flag is_dram_write){
  ...
  int bank_count=log2(DRAM_BANKS);
  int and_bits=DRAM_BANKS-1;
  int bank_number=(lineaddr>>4)&and_bits;
  int row_buffer_number=lineaddr>>4;
  //("%x\n",lineaddr);


  if (banks[bank_number].row_entry.valid== 0)
  {
      //printf("%d\n",bank_number);
	  banks[bank_number].row_entry.valid = 1;
	  banks[bank_number].row_entry.rowid= row_buffer_number;
      //printf("hit\n");
      delay=DRAM_T_ACT+DRAM_T_BUS+DRAM_T_CAS;
      //delay=100;
  }
  else if (banks[bank_number].row_entry.rowid == row_buffer_number)
  {
  ...
  {
	  banks[bank_number].row_entry.rowid = row_buffer_number;
      delay=DRAM_T_ACT+DRAM_T_PRE+DRAM_T_BUS+DRAM_T_CAS;
      //delay=145;
  }
  else
  {
