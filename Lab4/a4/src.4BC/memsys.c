...
uns64 memsys_access_modeBC(Memsys *sys, Addr lineaddr, Access_Type type){
  ...
  else if(needs_dcache_access)
  {
      ...
      if(outcome==MISS)
      {
          ...
          {
              int t=memsys_L2_access(sys,sys->dcache->last_evicted_line.tag, TRUE);
...
uns64   memsys_L2_access(Memsys *sys, Addr lineaddr, Flag is_writeback){
  ...
  if(outcome==MISS)
  {
      ...
      {
          //printf("Mem %d\n",count);
          int t=dram_access(sys->dram,sys->l2cache->last_evicted_line.tag, TRUE);
