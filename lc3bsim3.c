...
/************************* MEM_stage() *************************/
int v_mem_ld_cc, v_mem_ld_reg, mem_drid, mem_pc_mux, trap_pc, target_pc;
void MEM_stage() {

  int ii,jj = 0;
  int DCACHE_EN       = Get_DCACHE_EN(PS.MEM_CS) & PS.MEM_V;
  int DCACHE_RW       = Get_DCACHE_RW(PS.MEM_CS);
  int isWord          = Get_DATA_SIZE(PS.MEM_CS);
  int DCACHE_ADDR     = PS.MEM_ADDRESS;
  int isHighByte      = PS.MEM_ADDRESS & 0x0001;

  int WE0             = DCACHE_RW & (!isHighByte);  
  int WE1             = DCACHE_RW & (isWord ^ isHighByte); /* If asked for word, then isHighByte should == 0 */
  
  int writeDATA       = Low16bits(PS.MEM_ALU_RESULT);
  
  /* write high byte */
  if(isWord == FALSE && isHighByte == TRUE) {  
    writeDATA = Low16bits((writeDATA & 0x00FF) << 8);
  
  /* write to low byte */
  } else if(isWord == FALSE && isHighByte == FALSE) {
    writeDATA = Low16bits(writeDATA & 0x00FF);
  }

  int readDATA = 0;
  int DCACHE_R; /* When asserted, mem access is complete */
  if(DCACHE_EN == TRUE) {
    dcache_access(DCACHE_ADDR, &readDATA, writeDATA, &DCACHE_R, WE0, WE1);
  }

  /* Read only the high byte (as requested) */
  if(isWord == FALSE && isHighByte == FALSE) {
    readDATA = Low16bits(readDATA & 0x00FF);
    /* SEXT byte of read data */
    if( (readDATA & bit7_set) == 1) { 
      readDATA = Low16bits(readDATA & LOW_BYTE_SEXT); 
    }
  /* Read only the low byte (and sign extend) */
  } else if(isWord == FALSE && isHighByte == TRUE) {
    readDATA = Low16bits( (readDATA >> 8) & 0x00FF); 
    /* SEXT[byre of readDATA] */
    if( (readDATA & bit7_set) == 1) { 
      readDATA = Low16bits(readDATA & LOW_BYTE_SEXT); 
    }
  }
  ...
  /* Update PC, and handle special cases */
  /* Branch actions*/
  if(PS.MEM_V == TRUE && Get_BR_OP(PS.MEM_CS) == TRUE) {
    int IR_CC = ((PS.MEM_IR & 0x0E00) >> 9) & PS.MEM_CC;
    if(IR_CC != 0 ) {
      mem_pc_mux = 1; /* Choose Target_PC */
    } else {
      mem_pc_mux = 0; /* Choose PC+2 */
    }

  /* JSR/JSRR or Jump */
  } else if(PS.MEM_V == TRUE && Get_UNCOND_OP(PS.MEM_CS) == TRUE) {
      mem_pc_mux = 1; /*Target_PC */

  /* TRAP */
  } else if(PS.MEM_V == TRUE && Get_TRAP_OP(PS.MEM_CS) == TRUE) {
      mem_pc_mux = 2; /*Trap_PC */
  ...
  /* Push any 'read data' to new ps, else push 0 as we're still waiting */
  if(DCACHE_EN == TRUE && DCACHE_R == TRUE) {
  ...
  /* SR Latches Valid Mem....*/
  NEW_PS.SR_V = PS.MEM_V & ((DCACHE_EN & DCACHE_R) | (!DCACHE_EN));
...
void AGEX_stage() {
  /****MEM ADDRESS OUTPUT *****/
  int ADDR1_OUT;
  if(Get_ADDR1MUX(PS.AGEX_CS) == FALSE) {
      ADDR1_OUT = PS.AGEX_NPC;
  } else {    
      ADDR1_OUT = PS.AGEX_SR1;
  }
  ...
  switch(Get_ADDR2MUX(PS.AGEX_CS)) {
    ...
    case 1:         /* SEXT[IR[5:0]] */
        ADDR2_OUT = Low16bits(PS.AGEX_IR & 0x003F);
        
        /* SEXT if Neg # */
        if((ADDR2_OUT & 0x0020)>>5 == 1) { 
          ADDR2_OUT = Low16bits( (ADDR2_OUT | (~0x003F)) ); 
        }
        break;

    case 2:         /* SEXT[IR[8:0]] */
        ADDR2_OUT = Low16bits(PS.AGEX_IR & 0x01FF);
        
        /* SEXT if Neg # */
        if((ADDR2_OUT & 0x0100)>>8 == 1) {
          ADDR2_OUT = Low16bits( (ADDR2_OUT | (~0x01FF)) ); 
        }
        break;

    case 3:         /* SEXT[IR[10:0]] */
        ADDR2_OUT = PS.AGEX_IR & 0x07FF;

        /* SEXT if Neg #*/
        if((ADDR2_OUT & 0x0400)>>10 ==1) {
           ADDR2_OUT = Low16bits( (ADDR2_OUT | (~0x07FF)) ); 
        }
        break;
    default:
        ADDR2_OUT = 0;
  ...
  if(Get_LSHF1(PS.AGEX_CS)==TRUE) {
    LSHF1_OUT = Low16bits(ADDR2_OUT<<1);
  } else {
    LSHF1_OUT = Low16bits(ADDR2_OUT);
  }

  int MEM_ADDRESS;
  if(Get_ADDRESSMUX(PS.AGEX_CS)==FALSE) {
    /* LSHF(ZEXT(IR[7:0]),1)) */
    MEM_ADDRESS = Low16bits((PS.AGEX_IR & 0x00FF) << 1);
  } else {
    /*output of adder -> Mem Address */
    MEM_ADDRESS = Low16bits(ADDR1_OUT + LSHF1_OUT);
  }

  /*******ALU Outputs ******/
  int ALU_inB;
  if(Get_SR2MUX(PS.AGEX_CS) == FALSE) {
    /* SR2 -> ALU (B) */  
    ALU_inB = Low16bits(PS.AGEX_SR2);

  } else {
    /* SEXT(IR[4:0])-> ALU(B) */
    ALU_inB = Low16bits(PS.AGEX_IR & 0x001F);
    /* SEXT If -# */
    if( (ALU_inB & 0x0010)>>4 == 1 ) {
      ALU_inB = Low16bits((ALU_inB | 0xFFE0));
    }
  }

  int ALU_inA = PS.AGEX_SR1;
  int ALU_OUT;
  switch(Get_ALUK(PS.AGEX_CS)) {
    case 0:                     /* ADD */
          ALU_OUT = Low16bits(ALU_inA + ALU_inB);
          break;
  ...
  /********SHF Unit ********/
  int SHF_IN = PS.AGEX_SR1;
  int SHF_TYPE = (PS.AGEX_IR & 0x0030) >> 4;
  int SHF_AMM  = (PS.AGEX_IR & 0x000F);
  int SHF_OUT;

  switch(SHF_TYPE) {
    case 0:                   /* Left Logical Shift */
          SHF_OUT = Low16bits(SHF_IN << SHF_AMM);
          break;
    case 1:                   /* Right Logical Shift */
          SHF_OUT = Low16bits(SHF_IN >> SHF_AMM);
          break;
    case 3:                   /* Right Arithmatic Shift */
        /* If not a negative Number, simply right shift logical */
        if( (SHF_IN & 0x8000) != 0x8000) {
            SHF_OUT = Low16bits(SHF_IN >> SHF_AMM);
        ...
        /* Negative number.... */
            ...
            SHF_OUT = SHF_IN;
            for(i=0; i<SHF_AMM; i++) {
                /* Last bit 1 'or' number shifted right logically
                    aka, keep leading bits be 1 not 0               */
                SHF_OUT = Low16bits( (SHF_OUT >> 1) | 0x8000 );
            }
        }
        break;
    default:
  ...
  int MEM_ALU_RESULT;
  if(Get_ALU_RESULTMUX(PS.AGEX_CS) == FALSE) {
    MEM_ALU_RESULT = Low16bits(SHF_OUT);
  } else {
    MEM_ALU_RESULT = Low16bits(ALU_OUT);
 }
...
void DE_stage() {
  ...
  CONTROL_STORE_ADDRESS  =  (PS.DE_IR & 0xF800)>>10; /*Get bits 15:11 into 5:1 */
  CONTROL_STORE_ADDRESS |=  (PS.DE_IR & 0x0020)>>5; /* Move bit 5 into bit[0] */
  
  int SR1_REG = (PS.DE_IR & 0x01C0) >> 6;

  /* Get SR2_REG */
  int IR13 = (PS.DE_IR & 0x2000) >> 13;
  int SR2_REG;
  if(IR13 == TRUE) {
    SR2_REG = (PS.DE_IR & 0x0E00) >> 9; /* SR2_REG = IR[11:9] */
  } else {
    SR2_REG = (PS.DE_IR & 0x0007);      /* SR2_REG = IR[2:0] */
  }
  
  /* DRMUX controls wheter IR[11:9] or value of 7 is latched to AGEX.DRID */
  int DRID;
  if( Get_DRMUX(CONTROL_STORE[CONTROL_STORE_ADDRESS]) == FALSE ) {
      DRID = (PS.DE_IR & 0x0E00) >> 9; /* IR[11:9] */
  } else {
      DRID = 7;
  }

  /***** DEPENDENCY STALLS *****/
  if(Get_DE_BR_OP(CONTROL_STORE[CONTROL_STORE_ADDRESS]) == TRUE ) {
    dep_stall = v_agex_ld_cc | v_mem_ld_cc | v_sr_ld_cc;
  }

  int needSR1 = Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]);
  if(needSR1 == TRUE) {
    if(       (v_agex_ld_reg == TRUE) && (SR1_REG == agex_drid)) {
      dep_stall = TRUE;
    } else if((v_mem_ld_reg  == TRUE) && (SR1_REG == mem_drid)) {
      dep_stall = TRUE;
    } else if((v_sr_ld_reg   == TRUE) && (SR1_REG == sr_drid)) {
      dep_stall = TRUE;
    }
  }
  ...
  if(needSR2==TRUE) {
    if(       (v_agex_ld_reg == TRUE) && (SR2_REG == agex_drid) ) {
      dep_stall = 1;
    } else if ((v_mem_ld_reg  == TRUE) && (SR2_REG == mem_drid)) {
      dep_stall = 1;
    } else if ((v_sr_ld_reg   == TRUE) && (SR2_REG == sr_drid)) {
      dep_stall = 1;
    }
  }
  dep_stall &= PS.DE_V;
  
  /* insert a bubble if needed */
  if(dep_stall == TRUE) {
    NEW_PS.AGEX_V = 0;
  }

  /* BR_STALL */
  if(PS.DE_V == TRUE && Get_DE_BR_STALL(CONTROL_STORE[CONTROL_STORE_ADDRESS]) == TRUE) {
    v_de_br_stall = TRUE;
  } else {
    v_de_br_stall = FALSE;
  }

  /* Load reg AND update condition codes*/
  if(v_sr_ld_reg == TRUE) {
  ...
  if (LD_AGEX) {
    ...
    NEW_PS.AGEX_SR1   = REGS[SR1_REG];
    NEW_PS.AGEX_SR2   = REGS[SR2_REG];
    NEW_PS.AGEX_CC      = (N<<2) | (Z<<1) | P;
    NEW_PS.AGEX_DRID  = DRID;

    /* False if control Instruct, dependency, or DE false */
...
void FETCH_stage() {
  ...
  /* Calculate New PC, init to current PC */
  int newPC;
  switch(mem_pc_mux) {
    case 0:               /* PC += 2 */
          if(v_mem_br_stall == FALSE) {
            newPC = PC + 2;
          } else {
            newPC = PC;
          }
  ...
  /* load latches if no stalls in memory */
  if((!(mem_stall | dep_stall)) == TRUE) {
    NEW_PS.DE_V = icache_r & (!isStall);   

    NEW_PS.DE_IR = Instruction;
    NEW_PS.DE_NPC = PC + 2;
  }
 
  /* Load new PC...if no stalls and we've gotten our icache */
  if(   ( !(v_de_br_stall | v_agex_br_stall) & icache_r ) | (v_mem_br_stall) == TRUE ) {
    PC = newPC;
  }
}  
