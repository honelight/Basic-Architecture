...
void decode_instruction(){
  ...
  INST.amount4         = get_bits( INST.IR, 4, 0);
  INST.trapvect8       = get_bits( INST.IR, 8, 0);
  INST.B               = get_bits( INST.IR, 11, 11);
  INST.PCoffset11      = get_bits( INST.IR, 11, 0);
}
...
/************************************************************/
 ...
 void execute_ADD(){
    if(INST.A==0)
    {
        INST.DR=INST.SR1+INST.SR2;
    }
    else if(INST.A==1)
    {
        int imm=SEXT(INST.imm5,8);
        INST.DR=INST.SR1+imm;
    }
    setcc(INST.DR);
 }
 ...
 void execute_BR(){
    ...
    {
         int pc_offset = LSHF(SEXT(INST.PCoffset9, 7),1);
         NEXT_LATCHES.PC += pc_offset;
    }

 }

 void execute_LDB(){
    INST.DR=SEXT(read_word(Low16bits(INST.BaseR+SEXT(INST.boffset6,10))),8);
    setcc(INST.DR);
 }


 void execute_STB(){
    int output_memory=INST.BaseR+SEXT(INST.boffset6,10);
    write_word(Low16bits(output_memory),INST.DR);
 }
 ...
 void execute_JSR(){
    ...
    if(INST.B==0)
    {
        NEXT_LATCHES.PC=INST.BaseR;
    }
    else
    {
        NEXT_LATCHES.PC+=LSHF(SEXT(INST.PCoffset11,5),1);
    }
 }
 ...
 void execute_AND(){
    ...
    {
        INST.DR=INST.SR1 & SEXT(INST.imm5,11);
    }
    setcc(INST.DR);
 ...
 void execute_LDW(){
    INST.DR=read_word(INST.BaseR+LSHF(SEXT(INST.offset6,10),1));
    setcc(INST.DR);
 ...
 void execute_STW(){
    write_word(INST.BaseR+LSHF(SEXT(INST.offset6,10),1),INST.DR);
 }
 ...
 void execute_SHF(){
    if(INST.D==0)
    {
        INST.DR=LSHF(INST.SR1, INST.amount4);
    }
    else
    {
        if(INST.A==0)
        {
            INST.DR=RSHF(INST.SR1,INST.amount4,0);
        }
        else
        {
            INST.DR=RSHF(INST.SR1,INST.amount4,INST.SR1>>15);
        }
    }
 }
 ...
 void execute_LEA(){
    INST.DR=NEXT_LATCHES.PC+LSHF(SEXT(INST.PCoffset9,7),1);
 }
