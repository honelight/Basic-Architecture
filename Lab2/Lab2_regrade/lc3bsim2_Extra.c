...
int GetGATE_MUL(int *x)      { return(x[GATE_MUL]); }
...
static int memory_value=0;
int PC_gate,ALU_gate,SHF_gate, MARMUX_gate,MDR_gate,MUL_gate;
static int mul1=1;
static int mul2=0;
...
void eval_micro_sequencer() {
   ...
   int next_state=(j_code&0x0038)|((branch_bit|J_2)<<2)|((ready_bit|J_1)<<1)|((addr_mode_bit|J_0));


   if(GetIRD(CURRENT_LATCHES.MICROINSTRUCTION)==0)
   {
      int output_bits=0;
      if(CURRENT_LATCHES.STATE_NUMBER==10 && mul_count==1)
      {
          next_state=18;
      }
      NEXT_LATCHES.STATE_NUMBER=Low16bits(next_state);
      for(output_bits=0; output_bits<CONTROL_STORE_BITS;output_bits++)
      {
          NEXT_LATCHES.MICROINSTRUCTION[output_bits]=Low16bits(CONTROL_STORE[next_state][output_bits]);
      }
      NEXT_LATCHES.STATE_NUMBER=Low16bits(next_state);
   }
   else
...
void eval_bus_drivers() {
   ...
   if(GetGATE_MUL(CURRENT_LATCHES.MICROINSTRUCTION)==1)
   {
       mul_count++;
       if(mul_count==2)
       {
           int SR1=Low16bits(CURRENT_LATCHES.REGS[(CURRENT_LATCHES.IR & 0X01FF)>>6]);
           int SR2_output;
           SR2_output=Low16bits(CURRENT_LATCHES.REGS[(CURRENT_LATCHES.IR & 0x0007)]);
           MUL_gate=SR1*SR2_output;
       }
   }
}
...
void drive_bus() {
   ...
   if(GetGATE_PC(CURRENT_LATCHES.MICROINSTRUCTION)==1)
    ...
    BUS=SHF_gate;
   else if(GetGATE_MUL(CURRENT_LATCHES.MICROINSTRUCTION)==1)
   {
       if(mul_count==2)
       {
           BUS=MUL_gate;
