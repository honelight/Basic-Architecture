...
int PC_result, MARMUX_result, SHF_result, MDR_result,ALU_result, MUL_result;
...
void eval_bus_drivers() {
   ...
   */
   ...
   int gateMUL= GetGATE_MUL(CURRENT_LATCHES.MICROINSTRUCTION);
   ...
   if(gateMUL)
   {
       ...
       if(mul_count==2)
       {
           ...
           int SR2_output;
           SR2_output=Low16bits(CURRENT_LATCHES.REGS[(CURRENT_LATCHES.IR & 0x0007)]);
           MUL_result=SR1*SR2_output;
           //printf("%d %d %d\n",SR1,SR2_output,MUL_result);
       }
   }
