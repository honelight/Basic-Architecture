...
void eval_micro_sequencer() {
  ...
  int condition_code_1= (condition_code & 0x0010)>>1;
  ...
  if(ird_code==0)//move to a next state
  {
      ...
      int addr_mode_bit=(condition_code_1 & condition_code_0 & IR11_code);

      int next_state_code=(j_code & 0x0038) | ((j_code & 0x0004) | branch_bit) |
      ((j_code & 0x0002) | ready_bit) | ((j_code & 0x0001)|addr_mode_bit);
   ...
   */
   if(GetMIO_EN(CURRENT_LATCHES.MICROINSTRUCTION))
   {
       ...
       else if(cycle_count==5)
       {
           ...
           int read_or_write=GetR_W(CURRENT_LATCHES.MICROINSTRUCTION);
           int dataSize=GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION);
           int mar_we_logic=CURRENT_LATCHES.MAR & 0x0001;

           if(read_or_write)
           {
               if(dataSize)
               {
                   write_word(CURRENT_LATCHES.MAR, Low16bits(CURRENT_LATCHES.MDR));
               }
               else
               {
                   write_byte(CURRENT_LATCHES.MAR, Low16bits(CURRENT_LATCHES.MDR & 0xFF));
               }
           }
           else
           {
               if(dataSize)
               {
                   memory_value=Low16bits((read_word(CURRENT_LATCHES.MAR))&0xFFFF);
               }
               else
               {
...
void eval_bus_drivers() {
   ...
   */
   ...
   if(gateMARMUX)
   {
       ...
       int adder1=GetADDR1MUX(CURRENT_LATCHES.MICROINSTRUCTION);
       int adder2=GetADDR2MUX(CURRENT_LATCHES.MICROINSTRUCTION);
       int get_LSHF1=GetLSHF1(CURRENT_LATCHES.MICROINSTRUCTION);
       ...
       if(MARMUX_option==0)
       {
           MARMUX_result=CURRENT_LATCHES.IR&0x00FF;
           if(MARMUX_result & 0x0080)
           {
               MARMUX_result=MARMUX_result | 0x007F;
           }
           MARMUX_result=Low16bits(MARMUX_result<<1);
       }
       else if(MARMUX_option==1)
       {
           ...
           else if(adder2==1)
           {
               ...
               adder2_result=SEXT(CURRENT_LATCHES.IR, 0x001F);
           }
           else if(adder2==2)
           {
               //find the shift 6 bits from [8:0]
               adder2_result=SEXT(CURRENT_LATCHES.IR, 0x01FF);
           }
           else if(adder2==3)
           {
               //find the shift 6 bits from [11:0]
               adder2_result=SEXT(CURRENT_LATCHES.IR, 0x07FF);
           }
           ...
           MARMUX_result=Low16bits(adder2_result+adder1_result);
       }
   ...
   if (gateMDR)
   {
       ...
       if(word_size==0)
       {
           ...
           {
               MDR_result=SEXT(CURRENT_LATCHES.MDR>>8,8);
           }
           else
           {
               MDR_result=SEXT(CURRENT_LATCHES.MDR&0xFF,8);
           }
       }
       else
       {
   ...
   if(gateSHF)
   {
       int shift_type=(CURRENT_LATCHES.IR&0x003F)>>4;
       int shift_amount=(CURRENT_LATCHES.IR&0x000F);
       int SR1=getSR1();
       if(shift_type==0)
       {
           SHF_result=Low16bits(SR1<<shift_amount);
       }
   ...
   if(gateALU)
   {
       ...
       {
           SR2_output=Low16bits(CURRENT_LATCHES.IR & 0x001F);
       }
...
void latch_datapath_values() {
   ...
   */
    ...
    if((MDR_LD)&&(MIO_EN==0)&&(word_size==0)&&(MAR_byte==0))
    {
    ...
    else if((PC_LD)&&(PC_MUX==2))
    {
        ...
        {
            adder2_value=SEXT(CURRENT_LATCHES.IR, 0x001F);
        }
        else if(ADDr2MUX_value==2)
        {
            adder2_value=SEXT(CURRENT_LATCHES.IR, 0x01FF);
        }
        else if(ADDr2MUX_value==3)
        {
            adder2_value=SEXT(CURRENT_LATCHES.IR, 0x07FF);
        }
